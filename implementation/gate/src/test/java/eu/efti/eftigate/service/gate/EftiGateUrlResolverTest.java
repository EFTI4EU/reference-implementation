...
class EftiGateUrlResolverTest {
    ...
    void shouldResolveGates_WhenCountryIndicatorsAreGiven(){
        //Arrange
        this.consignmentIdentifiersRequestDto.setEFTIGateIndicator(List.of("BE", "FR"));
        when(gateRepository.findByCountryIn(anyList())).thenReturn(List.of(frGateEntity, beGateEntity));

        //Act
        final List<String> destinationGates = eftiGateUrlResolver.resolve(consignmentIdentifiersRequestDto);
