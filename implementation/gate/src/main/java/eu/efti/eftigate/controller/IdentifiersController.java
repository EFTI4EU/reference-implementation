...
public class IdentifiersController implements IdentifiersControllerApi {
    ...
    @Override
    public ResponseEntity<RequestUuidDto> getIdentifiers(final @RequestBody ConsignmentIdentifiersRequestDto consignmentIdentifiersRequestDto) {
        log.info("POST on /getIdentifiers with param vehicleId {}", consignmentIdentifiersRequestDto.getVehicleID());
        return new ResponseEntity<>(controlService.createIdentifiersControl(consignmentIdentifiersRequestDto), HttpStatus.ACCEPTED);
    }
