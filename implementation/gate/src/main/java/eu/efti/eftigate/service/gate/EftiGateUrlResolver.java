...
public class EftiGateUrlResolver {
    ...
    public List<String> resolve(final ConsignmentIdentifiersRequestDto consignmentIdentifiersRequestDto) {
        final Map<CountryIndicator, GateEntity> destinationGatesIndicatorMap;

        if (CollectionUtils.isNotEmpty(consignmentIdentifiersRequestDto.getEFTIGateIndicator())){
            final List<CountryIndicator> requestedCountryIndicators = consignmentIdentifiersRequestDto.getEFTIGateIndicator().stream().map(CountryIndicator::valueOf).toList();
