...
import static eu.efti.edeliveryapconnector.constant.EDeliveryStatus.BAD_GATEWAY;
...
public class UilRequestService extends RequestService<UilRequestEntity> {
    ...
    public void manageQueryReceived(final NotificationDto notificationDto) {
        ...
        try {
            ...
            if (!isSubsetValid) {
                ...
                this.sendRequest(this.buildErrorRequestDto(notificationDto, EXTERNAL_ASK_UIL_SEARCH, ErrorCodesEnum.BAD_SUBSETS.getMessage(), ErrorCodesEnum.BAD_SUBSETS.name(), RequestType.UIL));
            ...
            this.sendRequest(this.buildErrorRequestDto(notificationDto, EXTERNAL_ASK_UIL_SEARCH, exceptionMessage, ErrorCodesEnum.XML_ERROR.name(), RequestType.UIL));
        } catch (IllegalArgumentException | IOException e) {
            log.error("Something went wrong when validating UILQuery from{}", content.getFromPartyId(), e);
            this.sendRequest(this.buildErrorRequestDto(notificationDto, EXTERNAL_ASK_UIL_SEARCH, e.getMessage(), ErrorCodesEnum.XML_ERROR.name(), RequestType.UIL));
        }
    }
    ...
    public void manageResponseReceived(final NotificationDto notificationDto) {
        ...
        try {
            ...
            handleInvalidResponse(body, exceptionMessage, BAD_GATEWAY, notificationDto, content);
        } catch (IllegalArgumentException | IOException e) {
            log.error("Something went wrong when validating UILResponse from{}", content.getFromPartyId(), e);
            handleInvalidResponse(body, e.getMessage(), INTERNAL_SERVER_ERROR, notificationDto, content);
        }
    }

    private void handleInvalidResponse(String body, String exceptionMessage, EDeliveryStatus badGateway, NotificationDto notificationDto, NotificationContentDto content) {
        final UILResponse uilResponse = getSerializeUtils().mapXmlStringToJaxbObject(body);
    ...
    private void manageResponseFromPlatform(final UilRequestDto uilRequestDto, final UILResponse uilResponse, final NotificationDto notificationDto) {
        ...
        if (uilRequestDto.getControl().isExternalAsk()) {
            reportingRequestLogService.logReportingRequest(uilRequestDto.getControl(), uilRequestDto, getGateProperties().getOwner(), getGateProperties().getCountry(), RequestTypeLog.UIL, ComponentType.PLATFORM, uilRequestDto.getControl().getPlatformId(), getGateProperties().getCountry(), GATE, uilRequestDto.getControl().getFromGateId(), eftiGateIdResolver.resolve(uilRequestDto.getControl().getFromGateId()), true);
        } else {
